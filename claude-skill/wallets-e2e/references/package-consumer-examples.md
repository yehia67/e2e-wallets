...
As verified on 2026-08-30, published MetaMask `0.1.0` depends on core `0.1.3` but imports EVM symbols
that core `0.1.3` does not export. There is no honest, working npm-only MetaMask version pair to pin
on that date. Recheck before generating an install command:
...
The replacement values are deliberately invalid. Do not replace them with `latest` without
performing the export check. Until a compatible pair is published, tell the user that npm MetaMask
consumption is blocked rather than presenting a source checkout as package usage.
...
## Leather package example available today

The currently published Leather/core pair can be installed directly, but core `0.1.3` predates
`createExtensionTest` and `withWalletReporting`:

```bash
npm install --save-dev @playwright/test @wallets-e2e/core@0.1.3 @wallets-e2e/leather@0.1.3
npx playwright install chromium
```
...
For the newer typed `switchNetwork`, automatic video attachment, success/failure screenshots, traces,
and `withWalletReporting`, wait for a compatible core/Leather release and use the same consumer
fixture/configuration pattern shown above. Do not pretend those exports exist in core `0.1.3`.

...
This setup requires published core exports `createExtensionTest`, `withWalletReporting`, and
`createWalletSteps`. Verify them in the installed package; core `0.1.3` does not contain the first
two, so the full reporting flow is blocked until a compatible release exists.
